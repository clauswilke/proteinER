...
// Instead:
// Extract overallFrequencies from existingFit.positionalFrequencies to obtain nuc1x4
posFreq = existingFit.positionalFrequencies;
nuc1x4 = {4,1};
for (k=0; k<4; k=k+1)
{
	nuc1x4[k] = (posFreq[k][0]+posFreq[k][1]+posFreq[k][2])/3;
}

PopulateModelMatrix ("MGLocalQ", nuc1x4);
vectorOfFrequencies = BuildCodonFrequencies1x4(nuc1x4); // this function has been added into FUBAR_tools.ibf
